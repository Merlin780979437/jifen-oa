...
//				showCurrentAtPos: 1 // bug; use maxDate instead
	...
	function buildPresetsMenu(classnameContext, options, onClick) {
		...
		function init() {
			...
			if ($.ui.menu.prototype.options.items === undefined) {
				menuItemWrapper = {start: '<li class="ui-menu-item"><a href="#">', end: '</a></li>'};
			} else {
				menuItemWrapper = {start: '<li class="ui-menu-item"><div>', end: '</div></li>'};
			}
			...
			//$menu.uimenu().data('ui-menu').delay = 0; // disable submenu delays
		}
	...
	function buildDateRangePicker($originalElement, instance, options) {
		var classname = 'comiseo-daterangepicker',
			...
			hSide = LEFT, // initialized to pick layout styles from CSS
			vSide = null;
